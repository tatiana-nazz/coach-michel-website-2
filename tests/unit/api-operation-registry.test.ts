...
const expectedProductionOperations = [
  {
...
describe('P4-S06 X-API-COVERAGE production operation registry', () => {
  it('binds all production-final HTTP operations exactly', () => {
    expect(expectedProductionOperations).toHaveLength(56);

    const methodCounts = expectedProductionOperations.reduce(
      (counts, operation) => {
    ...
    expect(methodCounts).toEqual({ GET: 33, POST: 18, PATCH: 5 });

    for (const expected of expectedProductionOperations) {
      expect(operationRegistry.get(expected.operationId)).toEqual(expected);
    }
  });

  it('preserves the separately governed SCR-ACC-001 credential-submission operation', () => {
  ...
  it('preserves the scaffold-era empty registry export', () => {
    ...
    expect(emptyOperationRegistry.get(expectedProductionOperations[0].operationId)).toBeUndefined();
  });
});
